...
<body style="font-family: 'Poppins', sans-serif;">
    <header id="navbar" class="fixed top-0 left-0 right-0 z-50 bg-transparent w-full transition-all duration-300 ease-in-out">
        <nav class="container mx-auto flex items-center justify-between p-2">
          ...
          <button id="menu-button" class="p-4 text-cyan-800 text-xl bg-transparent rounded-md md:hidden">
            <i class="fas fa-bars"></i>
          </button>
          
          <!-- Desktop Navbar Links -->
          <div class="hidden md:flex space-x-6 font-semibold">
            <a href="{{ url('/') }}" class="text-lg text-black hover:text-teal-700">Beranda</a>
            <a href="{{ route('articles.index') }}" class="text-lg text-black hover:text-teal-700">Artikel</a>
            <a href="{{ url('/toko') }}" class="text-lg text-black hover:text-teal-700">Toko</a>
            <a href="{{ url('/comingsoon') }}" class="text-lg text-black hover:text-teal-700">Aplikasi</a>
            <a href="{{ url('/kontak') }}" class="text-lg text-black hover:text-teal-700">Kontak</a>
          </div>
        </nav>
    </header>
    ...
    window.addEventListener('scroll', () => {
      if (window.scrollY > 0) {
        ...
        for (let i = 0; i < navbarLinks.length; i++) {
          navbarLinks[i].classList.remove('text-black');
          navbarLinks[i].classList.add('text-cyan-800');
        }

        navbarbtn.classList.remove('text-cyan-800');
        navbarbtn.classList.add('text-teal-500');
      } else {
        navbar.classList.remove('backdrop-blur-lg', 'bg-transparent', 'shadow-2xl');

        for (let i = 0; i < navbarLinks.length; i++) {
          navbarLinks[i].classList.remove('text-cyan-800');
          navbarLinks[i].classList.add('text-black');
        }

        navbarbtn.classList.remove('text-teal-500');
        navbarbtn.classList.add('text-cyan-800');
      }
    });
